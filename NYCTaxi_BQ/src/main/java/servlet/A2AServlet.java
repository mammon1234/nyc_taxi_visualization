...
public class A2AServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String areaB = request.getParameter("areaB");
        if (attr != null && stime != null && etime != null && areaA != null && areaB != null) {
            Date st = null;
            Date et = null;
            try {
                st = sfg.parse(stime);
            ...
            String[] locationsA = areaA.split("_");
            String[] locationsB = areaB.split("_");

            String sql = "SELECT\n" + attr +
                    " FROM\n" +
                    "  [NYC.Trips]\n" +
                    "  WHERE\n" +
                    "  pickup_latitude > \""+ locationsA[0] +"\"\n" +
                    "  AND pickup_latitude < \""+ locationsA[2] +"\"\n" +
                    "  AND pickup_longitude > \""+ locationsA[3] +"\"\n" +
                    "  AND pickup_longitude < \""+ locationsA[1] +"\"\n" +
                    "  AND dropoff_latitude > \""+ locationsB[0] +"\"\n" +
                    "  AND dropoff_latitude < \""+ locationsB[2] +"\"\n" +
                    "  AND dropoff_longitude > \""+ locationsB[3] +"\"\n" +
                    "  AND dropoff_longitude < \""+ locationsB[1] +"\"\n" +
